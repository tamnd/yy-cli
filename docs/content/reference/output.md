...
describes what users get. Pick a format with `-o`, or let yy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
yy <command> -o table   # aligned columns for reading
yy <command> -o jsonl   # one JSON object per line, for piping
yy <command> -o json    # a single JSON array
yy <command> -o csv     # spreadsheet friendly
yy <command> -o tsv     # tab-separated
yy <command> -o url     # just the URL column
yy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
yy <command> --fields id,title,url
```
...
```bash
yy <command>            # a table, because this is a terminal
yy <command> | wc -l    # JSONL, because this is a pipe
```
